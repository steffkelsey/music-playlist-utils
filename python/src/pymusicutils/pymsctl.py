...
def pymsctl(
    input_file: Annotated[
        str, typer.Option(help="The json report with albums to search for")
    ],
    output_dir: Annotated[
        str, typer.Option(help="The directory to size downloaded music files")
    ],
    dry_run: Annotated[
        bool, typer.Option(help="Dry-run to print json results to stdout")
    ] = False,
    browser_json: Annotated[
        str,
        typer.Option(
            help="Path to the browser.json file from 'ytmusic browser' setup",
            envvar="BROWSER_JSON",
        ),
    ] = "$HOME/browser.json",
    cookies_txt: Annotated[
        str,
        typer.Option(
            help="Path to the cookies.txt from exporting cookies from browser in Netscape format",
            envvar="COOKIES_TXT",
        ),
    ] = "$HOME/cookies.txt",
):
    ...
    for a in exif_report["albums"]:
        ...
        for r in album_search_results:
            ...
            if score > 0.5:
                ...
                if "other_versions" in yt_album:
                    (
                        best_other_score,
                        best_other_track_match_indexes,
                        best_other_album,
                    ) = get_best_other_album(ytmusic, yt_album["other_versions"], a)
        ...
        if not matched:
            ...
            for r in artist_search_results:
                ...
                if r["artist"].lower() == a["artist"].lower():
                    ...
                    if (
                        not artist["albums"]["browseId"]
                        or not artist["albums"]["params"]
                    ):
                        artist_albums = artist["albums"]["results"]
                    else:
                        artist_albums = ytmusic.get_artist_albums(
                    ...
                    for artist_album in artist_albums:
                        ...
                        if (
                            "artists" not in artist_album
                            or len(artist_album["artists"]) == 0
                        ):
                        ...
                        if score > 0.4:
                            ...
                            # update the score
                            maybeAlbum = get_album_from_response(yt_album)
                            score, track_match_indexes = cmp_albums(
                                a, maybeAlbum, False
                            )
                            ...
                            if "other_versions" in yt_album:
                                (
                                    ...
                                    best_other_track_match_indexes,
                                    best_other_album,
                                ) = get_best_other_album(
                                    ytmusic, yt_album["other_versions"], a
                                )
                                if best_other_score > score:
                                    yt_album = best_other_album
                                    score = best_other_score
                            ...
                            if score > 0.8:
                                matched = True
                                playlistIdsToDownload.append(
                                    yt_album["audioPlaylistId"]
                                )
                                print(f"very good match on {a['album']}")
                                removed_matched_tracks(a, track_match_indexes)
                                break
        ...
        for t in a["tracks"]:
            print(
                f"missing track: {t['trackNumber']} - {t['title']} - {t['durationSeconds']}"
            )
        ...
        if len(a["tracks"]) > 0:
            ...
            found_tracks = []
            for t in a["tracks"]:
                maybe_index = bisect_left(
                    tracks_to_search_over,
                    t["title"].lower(),
                ...
                if best_t_score > 0.70:
                    ...
                    print(
                        f"+: {best_track['title']} - {best_track['duration_seconds']}"
                    )
    ...
    for vid in videoIdsToDownload:
        ret_val = subprocess.call(
            f'yt-dlp --cookies {cookies_txt} -P {output_dir} -o "%(album)s/%(autonumber)02d - %(track)s.%(ext)s" -x --audio-format mp3 --add-metadata "https://music.youtube.com/watch?v={vid}"',
            shell=True,
        )
...
def cmp_albums(album1, album2, ignore_tracks=True, verbose=False, threshold=0.5):
    ...
    result = subprocess.run(
        ["music-utils", "compare", "albums", "--data", base64_message],
        capture_output=True,
        text=True,
    )
...
def get_album_from_response(yt_album):
    ...
    if "tracks" in yt_album:
        for yt_track in yt_album["tracks"]:
            track = get_track_from_response(
                yt_track, album["artist"], album["totalTracks"]
            )
...
def cmp_tracks(track1, track2, type="track", threshold=0.7):
    ...
    # call music-utils
    result = subprocess.run(
        ["music-utils", "compare", "tracks", "--data", base64_message],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
